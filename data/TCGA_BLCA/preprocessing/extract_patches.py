...
#     with open('/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/misc_files/annotated_svs.txt','r') as f:
#         svs_paths = [i[:-1] for i in f.readlines()]
    with open('unextracted.pkl','rb') as f:
        unextracted_slides = pickle.load(f)
        svs_paths = [str(next(Path('/GPUFS/sysu_jhluo_1/wangyh/data/slides/TCGA_bladder/TCGA_bladder/').glob(f'*/{slide_id}.svs'))) for slide_id in unextracted_slides]
    ...
    for i,svs in enumerate(svs_paths):  #svs是一个svs图像路径的str
        ...
        #构造存放patch的目录，目录的结构为
        tile_path = svs.replace(str(Path(svs).parent),f'/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches/size{TILE_SIZE}')
        #提取操作
