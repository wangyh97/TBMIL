...
def filter_blood(img):
    ...
    return percent
#@jit(nopython=True)
def extract_patches(levels,scale,tile_path,slide_tiles,tumor_tiles,tumor=True):
    scale2level = {
        '10X':levels[0],
        '20X':levels[1]
    }
    level = scale2level[scale]
    image_arrays = []
    location = []
    if tumor:
        print(f'processing ---level {scale},tumor tiles')
    else:
        print(f'processing ---level {scale},non-tumor tiles')

    tile_folder_path = Path(tile_path).parent/scale
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if all((percent_1 >= 0.75,percent_2 >= 0.75)) and tile_masked.shape==(512,512,3):# 如果tile_masked形状不一致np.array转化后的dtype为object，无法使用h5保存
...
if __name__ == '__main__':
    ...
    parser.add_argument('--slide',type=str,help='single specific slide to extract,slide_name')
    parser.add_argument('--scale',type=int,help='extracting scale')
    ...
    # reading & saving paths ---CN过程中unnormed.pkl
    with open('/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches/size512/no_tiles.pkl','rb') as f:
        no_tiles = pickle.load(f)
    with open('/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches/size512/invalid_file.pkl','rb') as f:
        invalid_file = pickle.load(f)
    unextracted_patch_path = no_tiles + invalid_file
    if args.slide:
        unextracted_patch_path = [Path('/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches/size512/' + f'{args.scale}X/' + f'{args.slide}.svs')]
    unextracted_slide = {
        'slide_id':[],
        'scale':[]
    }
    for i in unextracted_patch_path:
        unextracted_slide['slide_id'].append(str(i.with_suffix('.svs')).split('/')[-1])
        unextracted_slide['scale'].append(str(i).split('/')[-2])
        
    batch_slide_id = unextracted_slide['slide_id'][args.size*(args.index-1):args.size*args.index]
    batch_scale = unextracted_slide['scale'][args.size*(args.index-1):args.size*args.index]
    
    patch_path = "/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches"

    extracted_case = []
    un_extracted_case = []

    for i,z in enumerate(zip(batch_slide_id,batch_scale)): # z包含slide_id & scale
        start = time.time()
        total_num = len(batch_slide_id)
        print(f"processing  {i+1}/{total_num}:------{z[0]}")
        
        #路径操作
        slide_id, scale = z
        slide_path = next(Path('/GPUFS/sysu_jhluo_1/wangyh/data/slides/TCGA_bladder/TCGA_bladder/').glob(f'*/{slide_id}'))
        xml_path = slide_path.with_suffix('.xml')   #返回一个path
        #构造存放patch的目录，目录的结构为
        tile_path = str(slide_path).replace(str(slide_path.parent),f'/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/raw_patches/size{TILE_SIZE}')
        #提取操作
        slide = get_slide(str(slide_path))
        ...
        try:
            ...
            try:
                extract_patches(levels,scale,tile_path,slide_tiles,tumor_tiles)
    #             extract_patches(levels,scale,tile_path,slide_tiles,non_tumor_tiles,tumor=False)
                extracted_case.append(slide_path)
            except Exception as e:
                un_extracted_case.append(slide_path)
        ...
        print(f"******{len(un_extracted_case)}/{len(batch_scale)} remain unextract******")
