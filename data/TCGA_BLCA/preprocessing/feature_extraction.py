...
#         state_dict = torch.load(f'simclr_feature_extractor/runs/{args.file_path}/checkpoints/model.pth')   #需要修改
        ...
        state_dict = torch.load(model_path[args.extractor])
        model = ResNetSimCLR(base_model='resnet18',out_dim=args.outdim)
        model_loader(model,state_dict,args)
    ...
    patches = list(Path(f'/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/CN_patches/size512/{args.scale}X').rglob('*.h5'))
    save_dir = '/GPUFS/sysu_jhluo_1/wangyh/data/TCGA_bladder/features/size512/' + f'{args.extractor}'
                ...
                for patch in tqdm(patches):
                    ...
                    try:
                        with h5py.File(patch,'r') as f:
                            ...
                            slide_features[slide_id] = {'features': feature_list,'coords':coords}

                    except Exception as e:
                        print(f'wrong in slide{slide_id}, error as {e}')
                        unextracted.append(slide_id)
                        np.save(os.path.join(save_dir,f'{args.scale}X_not_extracted.npy'),unextracted)
