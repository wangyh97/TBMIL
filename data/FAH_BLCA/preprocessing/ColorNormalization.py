...
with open('/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/misc_files/batch3_annotated_slides.txt','r') as f: # 从每个上传的batch读取
    svs_paths = [i[:-1] for i in f.readlines()]
slide_ids = [Path(i).stem for i in svs_paths]
cases=[]
for slide_id in slide_ids:
    cases.append(Path('/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/raw_patches/size512/10X/'+slide_id+'.h5'))
    cases.append(Path('/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/raw_patches/size512/20X/'+slide_id+'.h5'))
    
# cases = list(Path('/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/raw_patches/size512').rglob('*.h5')) #检索文件夹内全部cases一并处理
SIZE = 512
...
for case in tqdm(cases_select):
    with h5py.File(case, 'r') as f:
        ...
        cn_tiles_path = str(case).replace('/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/raw_patches/','/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/CN_patches/') #TODO
...
#         print(len(tiles))
        ...
        if len(tiles):
            try:
                ...
                logger.info(f"unprocessed case:{case},scale:{str(case.parent).split('/')[-1]}")
                continue
        else:
            logger.info(f'no patches in case:{case}, may be removed') 
        with open(f'/GPUFS/sysu_jhluo_1/wangyh/data/FAH_BLCA/CN_patches/unnormed_{args.batch}.pkl','wb') as f:
            pickle.dump(unnorm_tiles,f)
